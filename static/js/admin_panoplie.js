document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById('create-panoplie-btn').onclick = async function () {
        ...
        // This part needs to be handled differently as we can't use Jinja in JS files.
        // We will fetch the panoplies from the server.
        const response = await fetch('/admin/panoplies/api');
        const panoplies = await response.json();
        let exists = panoplies.some(p => p.name === name);
        ...
        if (!imgCheck.exists) {
            if (confirm("Aucune image trouvée pour cette panoplie. Voulez-vous uploader des images maintenant ?")) {
                ...
                let piecesToUpload = [];
                if (pieces === 8) {
                    piecesToUpload = allPieces;
                } else if (pieces === 4) {
                    if (type === "armure") {
                        piecesToUpload = piecesArmure;
                    } else if (type === "accessoire") {
                        piecesToUpload = piecesAccessoire;
                    }
                }

                // Génère le HTML des inputs
                let inputsHtml = piecesToUpload.map(piece => `
  <div style="margin-bottom:12px;">
    <label>${piece} :</label>
    <input type="file" name="file_${piece.replace(/ /g, "_")}" accept=".webp,.png,.jpg,.jpeg" required>
  </div>
`).join('');
                let overlay = document.createElement('div');
                overlay.id = "upload-panoplie-overlay";
                overlay.className = "edit-panoplie-overlay";
                overlay.style.display = "flex";
                overlay.innerHTML = `
  <div class="edit-panoplie-popup">
    <form id="upload-panoplie-img-form" enctype="multipart/form-data">
      <h2>Uploader les images de la panoplie</h2>
      ${inputsHtml}
...
`;
                ...
                document.getElementById('upload-panoplie-img-form').onsubmit = async function (e) {
                    ...
                    // Réaffiche le menu de création pour saisir les effets
...
const piecesArmure = ["Casque", "Plastron", "Gants", "Bottes"];
const piecesAccessoire = ["Collier", "Bracelet", "Bague", "Boucle d'oreille"];
const allPieces = [...piecesArmure, ...piecesAccessoire];

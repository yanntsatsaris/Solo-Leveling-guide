...
@characters_bp.route('/characters/add/check_image_folder')
@login_required
...
@characters_bp.route('/characters/add', methods=['POST'])
@login_required
def add_character():
    # Vérification des droits
    if not session.get('username') or not session.get('rights') or not ('Admin' in session['rights'] or 'SuperAdmin' in session['rights']):
...
@characters_bp.route('/characters/images_for/<type_folder>/<folder>')
@login_required
